...
from . import tsa_logic  # Import the modules with core logic/formatting
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "source_dot": {
                    "type": "string",
                    "description": "DOT code (format: XXX.XXX-XXX) of the Past Relevant Work (PRW).",
                },
                "residual_capacity": {
                    "type": "string",
                    "description": "Claimant's RFC level (e.g., SEDENTARY, LIGHT).",
                },
                "age": {
                    "type": "string",
                    "description": "Claimant's age category (e.g., ADVANCED AGE).",
                },
                "education": {
                    "type": "string",
                    "description": "Claimant's education category (e.g., HIGH SCHOOL).",
                },
                "target_dots": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Optional: Specific target DOT codes (format: XXX.XXX-XXX) suggested by VE.",
                },
            },
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "table_name": {
                    "type": "string",
                    "description": "Name of the table (e.g., 'DOT').",
                }
            },
            "required": ["table_name"],
        },
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "soc_code": {
                    "type": "string",
                    "description": "SOC code to search for (e.g., '15-1252')",
                }
            },
            "required": ["soc_code"],
        },
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "title": {
                    "type": "string",
                    "description": "Occupation title to search for (e.g., 'Software')",
                }
            },
            "required": ["title"],
        },
...
# Global DB Handler and BLS Handler instances (initialized in main)
db: Optional[DatabaseHandler] = None
bls_handler: Optional[BLSExcelHandler] = None
...
async def main(db_path: Path):
    ...
    try:
        ...
        # Warm up the cache with frequently accessed DOT codes
        from .generate_job_report import warm_up_cache

        # List of common DOT codes to pre-cache (most frequently accessed)
        common_dot_codes = [
            "211.462-010",  # Cashier
            "211.462-014",  # Cashier-Checker
            "299.367-014",  # Teacher Aide
            "209.587-034",  # Marker
            "920.687-014",  # Cleaner, Commercial or Institutional
            "290.477-014",  # Sales Clerk
            "311.677-014",  # Waiter/Waitress
            "920.687-134",  # Janitor
            "222.487-014",  # Order Clerk
            "361.684-014",  # Dishwasher
        ]

        # Warm up the cache (non-blocking way)
        import asyncio

        asyncio.create_task(asyncio.to_thread(warm_up_cache, db, common_dot_codes))
        logger.info("DOT code cache warm-up initiated.")
    except FileNotFoundError as e:
        logger.critical(
            f"Database file not found during DatabaseHandler init: {e}", exc_info=True
        )
    ...
    excel_file_path = (
        Path(__file__).parent / "DOTSOCBLS_Excel" / "bls_all_data_M_2024.xlsx"
    )
    try:
        bls_handler = BLSExcelHandler.get_instance(
            excel_file_path
        )  # Use get_instance for singleton
        ...
        logger.error(
            f"Error initializing BLSExcelHandler: {e}. BLS tools will be unavailable."
        )
        bls_handler = None  # Server will still run, but BLS tools won't work
    ...
    async def handle_list_prompts() -> list[types.Prompt]:
        ...
        return [
            types.Prompt(
                name="VE_MCP_Audit",  # The only supported prompt
                ...
                arguments=[
                    ...
                    types.PromptArgument(
                        name="transcript",
                        description="Full text of the hearing transcript.",
                        required=True,
                    ),
                    types.PromptArgument(
                        name="claimant_name",
                        description="Claimant identifier (optional).",
                        required=False,
                        default="Claimant",
                    ),
                ],
    ...
    async def handle_get_prompt(
        ...
        if name == "VE_MCP_Audit":
            ...
            try:
                ...
                logger.error(
                    "ve_audit_MCP_rag.PROMPT_TEMPLATE not found or inaccessible."
                )
                raise ImportError("Could not load the specified prompt template.")
            except KeyError as e:
                logger.error(
                    f"Error formatting ve_audit_MCP_rag.PROMPT_TEMPLATE - missing key? {e}"
                )
                raise ValueError("Error formatting transcript audit prompt template.")

            logger.debug(
                f"Generated prompt 'VE_MCP_Audit' for claimant: {args.get('claimant_name', 'Claimant')}"
            )
    ...
    async def tool_check_job_obsolescence(args, **kwargs):
        if "dot_code" not in args:
            raise ValueError("Missing required argument: dot_code")
    ...
    async def tool_analyze_transferable_skills(args, db, **kwargs):
        ...
        if missing:
            raise ValueError(f"Missing required arguments for TSA: {missing}")
    ...
    async def tool_generate_job_report(args, db, **kwargs):
        ...
        from .generate_job_report import get_job_data  # Data retrieval
        from .ve_logic import (
            get_job_analysis,
            generate_formatted_job_report,
        )  # Analysis and Formatting

        try:
            # Step 1: Get Raw Job Data
            raw_job_data = get_job_data(db, search_term)
            ...
            if not raw_job_data:
                ...
                return [
                    types.TextContent(
                        type="text",
                        text=f"No matching jobs found for search term: '{search_term}'.",
                    )
                ]

            logger.debug(f"Found raw job data for '{search_term}'")

            # Step 2: Perform Analysis on Raw Data
            # Pass None for hearing_date as it's not available in this tool's context
            analysis_data = get_job_analysis(raw_job_data, hearing_date_str=None)

            if "error" in analysis_data:
                logger.error(
                    f"Error during job analysis for '{search_term}': {analysis_data['error']}"
                )
                # Return the specific analysis error
                return [
                    types.TextContent(
                        type="text",
                        text=f"Error analyzing job data for '{search_term}': {analysis_data['error']}",
                    )
                ]

            logger.debug(f"Successfully analyzed job data for '{search_term}'")

            # Step 3: Format the Analysis Data
            try:
                report_text = generate_formatted_job_report(analysis_data)
                return [types.TextContent(type="text", text=report_text)]
            except Exception as format_err:
                logger.error(
                    f"Error formatting job report for '{search_term}': {format_err}",
                    exc_info=True,
                )
                # Return a specific error about formatting failure
                return [
                    types.TextContent(
                        type="text",
                        text=f"Successfully analyzed job data for '{search_term}' but encountered an error formatting the report: {str(format_err)}",
                    )
                ]

        except (
            ValueError
        ) as e:  # Catch errors from get_job_data or get_job_analysis specifically
            logger.error(
                f"ValueError in generate_job_report for '{search_term}': {e}",
                exc_info=True,
            )
            return [
                types.TextContent(
                    type="text",
                    text=f"Error processing job report for '{search_term}': Invalid input or data. {str(e)}",
                )
            ]
            ...
            logger.error(
                f"Unexpected error in tool_generate_job_report for '{search_term}': {error_type} - {error_msg}",
                exc_info=True,
            )
            return [
                types.TextContent(
                    type="text",
                    text=f"Unexpected error generating job report for '{search_term}': {error_type} - {error_msg}",
                )
            ]
    ...
    async def tool_analyze_bls_excel(args, bls_handler, **kwargs):
        ...
        try:
            if bls_handler.dataframe is not None:
                ...
                column_info = {
                    col: bls_handler.get_field_description(col)
                    for col in bls_handler.dataframe.columns
                }
                result = {
                    "status": "Success",
    ...
    async def tool_query_bls_by_soc(args, bls_handler, **kwargs):
        ...
        if "soc_code" not in args:
            raise ValueError("Missing required argument: soc_code")
        ...
        try:
            ...
            logger.error(
                f"Error querying BLS data by SOC code '{soc_code}': {e}", exc_info=True
            )
            return [
                types.TextContent(
                    type="text",
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text",
                    text=json.dumps(
                        {"query": title, "count": len(results), "results": results},
                        indent=2,
                    ),
                )
            ]
            ...
            logger.error(
                f"Error querying BLS data by title '{title}': {e}", exc_info=True
            )
            return [
                types.TextContent(
                    type="text",
    ...
    async def tool_write_file(args, **kwargs):
        ...
        try:
            ...
            workspace_root = Path(
                __file__
            ).parent.parent.parent  # Go up three levels from server.py
            ...
            if not target_file.resolve().is_relative_to(workspace_root.resolve()):
                logger.error(
                    f"Attempted to write file outside workspace: {target_file}"
                )
            ...
            logger.error(
                f"Unexpected error writing file {target_file}: {e}", exc_info=True
            )
            raise ValueError(
                f"An unexpected error occurred while writing the file: {str(e)}"
            )
    ...
    async def handle_call_tool(
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text",
                    text=f"Tool Error ({name}): Invalid input or value. {str(e)}",
                )
            ]
        except FileNotFoundError as e:
            logger.critical(
                f"FileNotFoundError calling tool '{name}': {e}", exc_info=True
            )
            return [
                types.TextContent(
                    type="text",
                    text=f"Server Configuration Error: Required file not found. {str(e)}",
                )
            ]
            ...
            return [
                types.TextContent(
                    type="text", text=f"Database Error ({name}): {str(e)}"
                )
            ]
            ...
            return [
                types.TextContent(
                    type="text",
                    text=f"Server Logic Error ({name}): Missing attribute or method. {str(e)}",
                )
            ]
        except Exception as e:
            ...
            return [
                types.TextContent(
                    type="text",
                    text=f"Error in {name}: {error_type} - {error_message}\n\nCheck server logs for detailed traceback.",
                )
            ]
