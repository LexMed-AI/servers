...
GRID_RULES_PATH = (
    Path(__file__).parent / "reference_json" / "medical_vocational_guidelines.json"
)
...
try:
    ...
    logger.error(
        f"Grid Rules JSON file not found at {GRID_RULES_PATH}. Grid rule application will not work."
    )
except json.JSONDecodeError:
    logger.error(
        f"Error decoding JSON from {GRID_RULES_PATH}. Grid rule application will not work."
    )
...
try:
    ...
    logger.error(
        f"SSR 82-41 JSON file not found at {SSR_82_41_PATH}. Detailed TSA rules may not be applied correctly."
    )
except json.JSONDecodeError:
    logger.error(
        f"Error decoding JSON from {SSR_82_41_PATH}. Detailed TSA rules may not be applied correctly."
    )
except Exception as e:
...
def _map_rfc_to_table_key(rfc_strength: str) -> Optional[str]:
    ...
    # Heavy/Very Heavy are handled by 204.00 text, not a table in the same format
        ...
        logger.warning(
            f"Unsupported RFC strength for Grid Rule table mapping: {rfc_strength}"
        )
        return None
...
def _map_age_category(age_input: str) -> str:
    ...
    if "advanced age" in age_lower or age_lower == "advanced":  # Matches "Advanced age"
        ...
        "closely approaching advanced" in age_lower
    ):  # Matches "Closely approaching advanced age"
        return "Closely approaching advanced age"
    elif (
        "younger" in age_lower and "45-49" in age_lower
    ):  # Matches "Younger individual age 45-49"
        return "Younger individual age 45-49"
    elif (
        "younger" in age_lower
    ):  # Matches "Younger individual age 18-44" or just "Younger individual"
        ...
        "closely approaching retirement" in age_lower
    ):  # Matches "Closely approaching retirement age" (Table 3)
...
def _map_education_level(education_input: str) -> str:
    ...
    if edu_lower == "illiterate":
        ...
        if "marginal" in edu_lower and "illiterate" in edu_lower:  # Table 3 specific
            ...
            "marginal" in edu_lower and "not illiterate" in edu_lower
        ):  # Table 2 specific
...
def apply_grid_rule(
    ...
    age_category: str,
    education_level: str,
    prw_skill_category: str,
    skills_transferable: bool,
    ...
    if GRID_RULES_DATA is None:
        return {
            "rule_id": None,
            "decision": "Error: Grid Rules data not loaded.",
            "reason": "GRID_RULES_DATA is None",
        }
    ...
    if not table_key:
        return {
            ...
            "reason": f"RFC '{rfc_strength}' does not map to a standard Grid table.",
        }
    ...
    if table_key == "204.00":
        ...
        return {
            "rule_id": "204.00 (Principle)",
            "decision": "Not disabled",
            "reason": "RFC allows Heavy or Very Heavy work (per 204.00 principle).",
        }
    ...
    table_section = next(
        (
            section
            for section in GRID_RULES_DATA.get("sections", [])
            if section.get("section_number") == table_key
        ),
        None,
    )
    if not table_section or "table" not in table_section:
        logger.error(
            f"Could not find table data for section {table_key} in GRID_RULES_DATA."
        )
        return {
            "rule_id": None,
            "decision": "Error: Grid Rules data structure unexpected.",
            "reason": f"Missing table for section {table_key}",
        }
    ...
    transferable_status = (
        skills_transferable
        if prw_skill_category.lower() not in ["unskilled", "none"]
        else False
    )
    formatted_prw = _format_previous_work_experience(
        prw_skill_category, transferable_status
    )

    logger.debug(
        f"Attempting Grid Rule match for: RFC Table={table_key}, Age='{mapped_age}', Edu='{mapped_education}', PRW='{formatted_prw}'"
    )
    ...
    for rule in rules:
        ...
        if rule.get("age") == mapped_age and education_matches and pwe_matches:
            # Check transferability part if PWE requires it
            if "transferable" in rule.get("previous_work_experience", ""):
                rule_is_transferable = "skills transferable" in rule.get(
    ...
    return {
        ...
        "reason": "No exact match found for the provided vocational profile.",
    }
...
def _extract_potential_skills(analysis_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        skills["work_fields"] = [
            wf.get("code")
            for wf in analysis_data.get("work_fields", [])
            if wf.get("code")
        ]
        # MPSMS - store as a list of codes
        skills["mpsms"] = [
            m.get("code") for m in analysis_data.get("mpsms", []) if m.get("code")
        ]

        # Validate extracted data (e.g., ensure SVP is a number)
        if not isinstance(skills.get("svp"), (int, float)):
            logger.warning(
                f"Invalid or missing SVP in analysis data: {analysis_data.get('job_title')}"
            )
    ...
    for key in ["data", "people", "things"]:
        if skills["worker_functions"][key] is None:
            logger.warning(
                f"Missing Worker Function '{key}' rating in analysis data for {analysis_data.get('job_title')}"
            )
...
def _evaluate_transferability(
    ...
    if not target_skills or not source_skills:
        return {
            "transferable": False,
            "reason": "Missing necessary skill data in source or target analysis.",
    ...
    if (
        not source_wf
        or not target_wf
        or None in source_wf.values()
        or None in target_wf.values()
    ):
        reasons.append(
            "Worker Function ratings missing or incomplete in source or target."
        )
    ...
    if reasons:
        return {
            "transferable": False,
            "reason": "Essential data missing for comparison: " + "; ".join(reasons),
            "checks_passed": checks_passed,
        }

    # --- 1. Check Exertion Level --- #
    target_exertion = target_analysis.get("physical_demands", {}).get("exertion_level")
    checks_passed["exertion_met"] = _is_exertion_within_rfc(
        target_exertion, rfc_strength
    )
    if not checks_passed["exertion_met"]:
        reasons.append(
            f"Target exertion ({target_exertion}) exceeds RFC ({rfc_strength})."
        )
    else:
        reasons.append(
            f"Target exertion ({target_exertion}) within RFC ({rfc_strength})."
        )
    ...
    if isinstance(source_svp, (int, float)) and isinstance(target_svp, (int, float)):
        if target_svp <= source_svp and (source_svp - target_svp <= 2):
            ...
            reasons.append(
                f"Target SVP ({target_svp}) compatible with Source SVP ({source_svp})."
            )
        else:
            reasons.append(
                f"Target SVP ({target_svp}) not compatible with Source SVP ({source_svp}) (max 2 level reduction rule)."
            )
    else:
        reasons.append(
            "Invalid SVP values for comparison."
    ...
    if (
        source_wf
        and target_wf
        and source_wf.get("data") == target_wf.get("data")
        and source_wf.get("people") == target_wf.get("people")
        and source_wf.get("things") == target_wf.get("things")
    ):
        # Ensure none were None implicitly
        if None not in [
            source_wf.get("data"),
            source_wf.get("people"),
            source_wf.get("things"),
        ]:
    ...
    if wfld_overlap or mpsms_overlap:
        ...
        if wfld_overlap:
            overlap_items = ", ".join(
                str(item) for item in source_wfld.intersection(target_wfld)
            )
            reason_overlap.append(f"Work Field overlap found ({overlap_items}).")
        if mpsms_overlap:
            overlap_items = ", ".join(
                str(item) for item in source_mpsms.intersection(target_mpsms)
            )
            reason_overlap.append(f"MPSMS overlap found ({overlap_items}).")
    ...
    # Construct the reasons string more carefully
    failure_reasons_list = []
    # Iterate through specific checks to build failure reasons
    if not checks_passed["exertion_met"]:
        failure_reasons_list.append(
            reasons[0] if reasons else "Exertion not met"
        )  # Assuming reasons[0] is for exertion
    if not checks_passed["svp_compatible"]:
        failure_reasons_list.append(
            reasons[1] if len(reasons) > 1 else "SVP not compatible"
        )  # Assuming reasons[1] is for SVP
    if not checks_passed["worker_functions_match"]:
        failure_reasons_list.append(
            reasons[2] if len(reasons) > 2 else "Worker functions do not match"
        )  # Assuming reasons[2] is for WF
    if not checks_passed["wfld_mpsms_overlap"]:
        failure_reasons_list.append(
            reasons[3] if len(reasons) > 3 else "No WFLD/MPSMS overlap"
        )  # Assuming reasons[3] is for overlap

    final_reason_str: str
    if is_transferable:
        final_reason_str = "Skills transferable based on criteria."
    else:
        if (
            not failure_reasons_list
        ):  # Generic failure if specific reasons weren't captured properly
            failure_reasons_list.append("One or more transferability criteria not met.")
        final_reason_str = "Skills not transferable to this specific job: " + "; ".join(
            failure_reasons_list
        )
    ...
    return_value: Dict[str, Union[bool, str, Dict[str, bool], Dict[str, Any]]] = {
...
async def perform_tsa_analysis(
    ...
    **Partially Implemented** - Grid Rule application added, but skill identification
    ...
    """
    logger.info(
        f"Performing TSA: PRW={source_dot_code}, RFC={rfc_strength}, Age={age_category}, Edu={education_level}"
    )

    # 1. Get Source Job Data & Analysis
    source_job_data = db_handler.get_job_by_code(
        source_dot_code
    )  # Assuming synchronous for now
    if not source_job_data:
        logger.error(
            f"TSA failed: Could not retrieve data for source DOT {source_dot_code}"
        )
    ...
    if "error" in source_analysis:
        logger.error(
            f"TSA failed: Error analyzing source DOT {source_dot_code}: {source_analysis['error']}"
        )
        return {
            "error": f"Error analyzing source DOT {source_dot_code}: {source_analysis['error']}"
        }

    source_svp = source_analysis.get("skill_level", {}).get("svp", 0)
    source_skill_category = source_analysis.get("skill_level", {}).get(
    ...
    is_prw_unskilled = (
        source_skill_category.lower()
        == config.svp_to_skill_level.get(1, "unskilled").lower()
    )
    if is_prw_unskilled:
    ...
    if not source_skills:
        ...
        return {
            "error": f"Could not extract potential skills from source DOT {source_dot_code}"
        }

    # Initialize transferability status and results
    skills_are_transferable = (
        False  # Overall status, True if *any* target is transferable
    )
    ...
    if target_dot_codes:
        logger.info(
            f"Evaluating transferability to specific targets: {target_dot_codes}"
        )
        for target_code in target_dot_codes:
            target_job_data = db_handler.get_job_by_code(target_code)
            if not target_job_data:
                logger.warning(
                    f"Could not find data for target DOT {target_code}. Skipping evaluation."
                )
            ...
            if "error" in target_analysis:
                logger.warning(
                    f"Could not analyze target DOT {target_code}: {target_analysis['error']}. Skipping evaluation."
                )
                target_job_evaluation_results.append(
                    {
                        "target_dot": target_code,
            ...
            evaluation_result = _evaluate_transferability(
                source_skills, target_analysis, rfc_strength, age_category
            )
            target_job_evaluation_results.append(
                {
                    "target_dot": target_code,
            ...
            if evaluation_result.get("transferable"):
                ...
                if (
                    transferability_reason
                    == "No specific target DOTs provided for evaluation."
                ):
        ...
        logger.info(
            "No specific target DOTs provided. General transferability assessment not implemented."
        )
    ...
    if grid_decision not in [
        None,
        "Error",
        "Not Applicable",
        "Rule Not Applicable",
    ]:  # Ensure grid_decision is compared correctly
        ...
        if skills_are_transferable:
            first_transferable_target_dot = (
                "details in target analysis"  # Default message is a string
            )
            if target_job_evaluation_results:  # Check if list is not empty
                # Find the first transferable target
                for target_eval in target_job_evaluation_results:
                    if isinstance(target_eval, dict):
                        evaluation = target_eval.get("evaluation")
                        if isinstance(evaluation, dict) and evaluation.get(
                            "transferable", False
                        ):
                            # Found a transferable target, use its dot code
                            first_transferable_target_dot = target_eval.get(
                                "target_dot", first_transferable_target_dot
                            )
                            break

            tsa_conclusion = f"Grid Rules do not directly apply ({grid_reason}), but skills were found transferable to other occupations within RFC limits (e.g., {first_transferable_target_dot}). Finding would likely be 'Not Disabled'."
        else:
    ...
    return {
        ...
        "grid_rule_application": {
            ...
            "reasoning": grid_reason
            or ("Rule applied successfully." if grid_rule_id else "No rule applied."),
        },
        "prw_details": {
